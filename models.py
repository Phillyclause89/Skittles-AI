...
def hist_inception_v3(width, height, frame_count, lr, output=9, model_name='sentnet_color.model'):
...
def resnext(width, height, frame_count, lr, output=9, model_name='sentnet_color.model'):
    ...
    net = tf.layers.conv.resnext_block(net, net, 16, 32)
    net = tf.resnext_block(net, 1, 32, 32, downsample=True)
    net = tf.resnext_block(net, net - 1, 32, 32)
    net = tf.resnext_block(net, 1, 64, 32, downsample=True)
    net = tf.resnext_block(net, net - 1, 64, 32)
...
def sentnet_LSTM_gray(width, height, frame_count, lr, output=9):
    ...
    # network = tf.input_data(shape=[None, 28, 28], name='input')
    network = tf.lstm(network, 128, return_seq=True)
